...
{
  # NOTE Use diagonal covariance matrices.
  dgp <- mog2_sim(N = N,
    pis = c(0.35, 0.65),
    mus = rbind(-2, 2),
  ...
  #  Sigmas = list(diag(c(0.4, 0.8)), diag(c(0.6, 0.3))),
  #  seed = 1000 + iter)

  ## Pure CAVI.
  t_cavi <- system.time({
    fit_cavi <- mog3_cavi(X = dgp$X, K = dgp$K,
  ...
  init_lin <- mog3_init(X = dgp$X, K = dgp$K,
    fixed_pars = list(alpha0 = 1, beta0 = 1, m0 = NULL, a0 = 1, b0 = 1),
  ...
  t_lin <- system.time({
    fit_lin <- mog3_cavi_amortized(vec_etas = vec_etas_lin,
  ...
  init_nn <- mog3_init(X = dgp$X, K = dgp$K,
    fixed_pars = list(alpha0 = 1, beta0 = 1, m0 = NULL, a0 = 1, b0 = 1),
  ...
  t_nn <- system.time({
    fit_nn <- mog3_cavi_amortized(vec_etas = vec_etas_nn, model_struct = model_struct_nn,
